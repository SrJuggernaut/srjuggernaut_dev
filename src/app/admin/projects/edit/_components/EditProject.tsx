...
import { getProject, projectDataSchema, updateProject } from '@/services/frontend/projects'
...
const EditProject:FC = () => {
  ...
  const formik = useFormik<ProjectData>({
    ...
    onSubmit: async (values) => {
      try {
        await updateProject(id as string, values)
        addAlert({ id: nanoid(), title: 'Proyecto actualizado', text: 'El proyecto se ha actualizado correctamente', severity: 'success' })
      } catch (error) {
  ...
  useEffect(() => {
    ...
    getProject(paramId)
      ...
      .catch((error) => {
        handleError(error, 'Error al obtener el proyecto', 'Ocurrio un error al obtener el proyecto. Por favor, intenta de nuevo.', 'error')
        router.push('/admin?tab=projects')
      })
      .finally(() => {
        setId(paramId)
  ...
  return (
    <>
      <Typography variant="h1" align="center"gutterBottom>Editar proyecto</Typography>
      <Box
        component="form"
