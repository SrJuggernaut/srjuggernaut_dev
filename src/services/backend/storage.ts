import { ADMIN_TEAM_ID, MEDIA_STORAGE_ID, MEDIA_STORAGE_NAME, SEO_IMAGES_STORAGE_ID, SEO_IMAGES_STORAGE_NAME } from '@/lib/env'
...
export const ensureMediaBucket = (() => {
  let existBucket = false
  return async () => {
    if (existBucket) return
    try {
      await storage.getBucket(MEDIA_STORAGE_ID)
    } catch {
      const permissions = [
        Permission.create(Role.team(ADMIN_TEAM_ID)),
        Permission.read(Role.any()),
      ...
      await storage.createBucket(MEDIA_STORAGE_ID, MEDIA_STORAGE_NAME, permissions, false, true, tenMB, undefined, Compression.Gzip, false, true)
    } finally {
      existBucket = true
    }
  }
